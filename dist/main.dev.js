...
inputDigit = function inputDigit(digit) {
  ...
  var currentDisplayValue = calculator.displayValue; // Creating a variable called waitingForSecondNumber which is equal to the current value of waitingForSecondNumber property of calculator
  // This lets us know whether we're inputting the first or second number into the calculator and sets it in a variable

  var currentWaitingForSecondNumber = calculator.waitingForSecondNumber; // If we have typed an operator key

  if (currentWaitingForSecondNumber === true) {
    // Overwrites the current number on the screen
    calculator.displayValue = digit; // Number typed, so no longer waiting for an second number
...
removeLeadingZero = function removeLeadingZero() {
...
inputDecimal = function inputDecimal(point) {
  ...
  if (calculator.waitingForSecondNumber === true) {
    ...
    calculator.displayValue = "0."; // A number is now on screen, so no longer waiting for the second number

    calculator.waitingForSecondNumber = false; // exit
...
inputOperator = function inputOperator(operatorClicked) {
  // Declare several variables
  // currentFirstNumber which is the current value of the calculator object firstNumber
  var currentFirstNumber = calculator.firstNumber; // This gets the current display value on the screen and puts it in a variable

  var currentDisplayValue = calculator.displayValue; // This gets if an operator is stored previously in the calculator object.

  var currentOperator = calculator.operator; // Converts the current display value to a floating point number (a number with decimal points even if they aren't shown)

  var inputValue = parseFloat(currentDisplayValue); // If the currentFirstNumber hasn't been set and inputValue isn't 0

  if (currentFirstNumber === null && !isNaN(inputValue)) {
    // set the firstNumber in the calculator object to the inputValue
    calculator.firstNumber = inputValue; // if the current operator is not null
  } else if (currentOperator) {
    // Print out currentFirstNumber, inputValue and currentOperator to the console
    console.log(currentFirstNumber, inputValue, currentOperator); // Declaring a variable result setting it equal to the result of the function calculate.
    // We give the function calculate the variables currentFirstNumber, inputValue and currentOperator

    var result = calculate(currentFirstNumber, inputValue, currentOperator); // Print out the result to the console

    console.log(result); // Updating the display to the result

    calculator.displayValue = result; // Updating the firstNumber to the result

    calculator.firstNumber = result;
  } // Tell the object we've clicked an operator


  calculator.waitingForSecondNumber = true; // Tell the object which operator we've clicked

  calculator.operator = operatorClicked; // Print out the calculator object

  console.log(calculator);
}; // A function to do maths on the numbers we've input


calculate = function calculate(firstNumber, secondNumber, operator) {
  // Print out which operator we've clicked (what type of maths we'll do)
  console.log(operator); // If we've clicked the plus button return the result of adding the 2 numbers

  if (operator === "add") {
    return firstNumber + secondNumber; // If we've clicked the minus button return the result of subracting the second number from the first one
  } else if (operator === "minus") {
    return firstNumber - secondNumber; // If we've clicked the multiply button return the result of multiplying the 2 numbers
  } else if (operator === "multiply") {
    return firstNumber * secondNumber; // If we've clicked the divide button return the result of dividing the 2 numbers
...
updateDisplay = function updateDisplay() {
  ...
  var display = document.querySelector(".calculator__screen"); //update the value of the element with the contents of 'displayValue'

  display.innerHTML = calculator.displayValue;
}; //calling the function updateDisplay


updateDisplay(); //3. Pressing keys
...
keys.addEventListener("click", function (event) {
  ...
  //below.
  var target = event.target; //Creating a variable called target which is equal to the target of the
  //event.
